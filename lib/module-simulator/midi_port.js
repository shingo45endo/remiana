...
export class MidiPort {
	...
	pushBytes(bytes) {
		...
		while (restBytes.length > 0) {
			...
			if (statusByte === 0xf7) {
				// Trashes EOX byte and restart parsing the rest buffer from MSB-set byte.
				restBytes = restBytes.slice(1);
				this._runningStatus = 0x00;
				continue;
				...
				if (restBytes[indexMsb] !== 0xf7) {
					// Trashes wrong bytes and restart parsing the rest buffer from MSB-set byte.
