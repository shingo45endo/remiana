	...
	pushBytes(bytes) {
		...
		while (restBytes.length > 0) {
			...
			if (statusByte === 0xf7) {
				...
				if (indexRealtime >= 0) {
					// Extracts and pushes the MSB-set byte as a real-time MIDI message and restarts parsing.
				...
				if (msbByte >= 0xf8) {
					// Cuts and pushes the MSB-set byte as a real-time MIDI message and restarts parsing.
