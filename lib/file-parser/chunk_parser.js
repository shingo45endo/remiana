...
export function parseChunk(buf, parentTags = [], logs = []) {
	...
	while (index + 8 <= buf.byteLength) {
		...
		if (isContainer) {
			if (!isBigEndian) {	// RIFF container
				...
				chunks.push({tag: type, tags: [...parentTags, type], container: {payload: buf.subarray(index, index + len - 4)}});
				...
				// Adds the payload as container.
				chunks.push({tag, tags: [...parentTags, tag], container: {payload: buf.subarray(index, index + len)}});
			...
			// Parses the payload as chunk.
			const chunk = parseContentsChunk(buf.subarray(index, index + len), tag, parentTags, logs);
			if (Array.isArray(chunk)) {	// Array means chunks.
				chunks.push(...chunk);
			} else {	// Object means content.
				console.assert(chunk && typeof chunk === 'object');
				chunks.push({tag, tags: [...parentTags, tag], content: chunk});
			}
	...
	if (index < buf.byteLength) {
		...
		chunks.push({tag, tags: [...parentTags, tag], content: {payload}});
