...
import {makeTempoMap} from './tempo_map.js';
import {makePortMap} from './port_map.js';
...
export function parseMidiFile(buf, options = {}, logs = []) {
	...
	if (buf.byteLength < 4 + 4 + 6 + 4 + 4) {	// 'MThd' + <length> + (<format> + <ntrks> + <division>) + 'MTrk' + <length>
	...
	if (macBinary) {
		...
		content.chunks.push({tag, tags: [tag], payload, content: {...macBinary}});
	...
	if (mthdNum > 0) {
		...
		// Makes tempo map and port map for each song.
		for (const song of content.songs) {
			song.tempoMap = makeTempoMap(song);
			song.portMap  = makePortMap(song);
		}
